...
| (PAbs (s,_))::ls -> let nenv = fn env ls in
...
let do_texify theo var s =
   ...
   let store_str s =
     for i = 0 to String.length s - 1 do store s.[i] done in
   ...
   | [< ' ('.' as c); str >] ->
       ...
       if !state = Alpha then (store '}');
       state := Non;
       for _ = 1 to !to_close do store '}' done;
       to_close:=0;
       store c;
       fn str
   | [< ' ('0'..'9' as c); str >] ->
...
let rename e =
  let rec fn depth env = function
	...
	let vars = List.filter (fun (d,_) -> d <> depth) vars in
	...
	let name =
          ...
          with Not_found -> try
              (*texify false true (expr_local e)*) raise Not_found (*FIXME*)
          with Not_found -> ("UCst"^(string_of_int n))
	in e, [-1, name]
    | EAtom(o,_) as e ->
	let tex, _ = get_rsyntax o in
...
let rec
  ...
  | _::_ ->
	...
	if level_leq llvl min_level then
	 ...
	 for _ = 1 to List.length stack do print_string "\171" done;
...
and do_bind lr the_binder al =
  ...
  and look c e = try
       ...
       | Oneq _,o -> match snd (get_rsyntax o) with
           Trivial | Mute | Free -> ()
         | Prefix (sy,_) -> sysaturate e1 sy
         | Infix (_,_,sy,_,_,_) -> sysaturate e1 ((Arg min_level)::sy)
...
and print_match stack e =
  ...
  and gn envl acc o n i =
    ...
    | _, _, EApp(EApp(EAtom(oc,_),a),fail) when is_case oc ->
	let case_name = get_name oc in
	let ob =
	  sym_get (String.sub case_name 5 (String.length case_name - 5)) in
	let nb_args =
...
| Path (p,e1) -> print_string "[[";
                ...
                print_string "]]"
| Syntax _ -> print_string "_a_syntax_"
